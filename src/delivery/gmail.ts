...
<body style="margin: 0; padding: 0; background-color: #f5f7fa; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif;">
          ...
          <tr>
            <td style="background-color: #f8f9fa; padding: 24px 40px; border-top: 1px solid #e9ecef; text-align: center;">
              <p style="margin: 0; color: #6c757d; font-size: 13px; line-height: 1.5;">
                🤖 Generated with AI &middot; ${new Date().toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' })}
              </p>
            </td>
          </tr>
  ...
  private markdownToHtml(markdown: string): string {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      // Heading 2 (Main Sections) — gradient left-border card
      if (line.startsWith('## ')) {
        if (inList) { html += '</ul>'; inList = false; }
      ...
      // Heading 3 (Subsections / theme titles)
      else if (line.startsWith('### ')) {
        if (inList) { html += '</ul>'; inList = false; }
        const title = this.escapeHtml(line.substring(4));
        html += `<h3 style="margin: 20px 0 12px 0; color: #4a5568; font-size: 18px; font-weight: 600;">${title}</h3>`;
      }
      // Bullet list
